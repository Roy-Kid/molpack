...
fn compute_f_cached_path_matches_fresh() {
    ...
    let fdist_a1 = sys_a.eval.fdist;
    let frest_a1 = sys_a.eval.frest;

    force_cache_miss(&mut sys_b);
    let f_b1 = compute_f(&x, &mut sys_b);
    let fdist_b1 = sys_b.eval.fdist;
    let frest_b1 = sys_b.eval.frest;
    ...
    assert_eq!(sys_a.eval.fdist.to_bits(), sys_b.eval.fdist.to_bits());
    assert_eq!(sys_a.eval.frest.to_bits(), sys_b.eval.frest.to_bits());
}
...
fn radii_swap_between_fg_and_f_cached_matches_fresh() {
    ...
    let scaled: Vec<F> = sys_a.eval.radius_ini.iter().map(|r| r * 1.2).collect();
    let unscaled = sys_a.eval.radius_ini.clone();

    // --- A: cached path ---
    sys_a.eval.radius = scaled.clone();
    sys_a.sync_atom_props();
    let mut g_a = vec![0.0; x.len()];
    let _ = compute_fg(&x, &mut sys_a, &mut g_a);

    sys_a.eval.radius = unscaled.clone();
    sys_a.sync_atom_props();
    let f_a = compute_f(&x, &mut sys_a);

    // --- B: always rebuild ---
    sys_b.eval.radius = scaled.clone();
    sys_b.sync_atom_props();
    let mut g_b = vec![0.0; x.len()];
    force_cache_miss(&mut sys_b);
    let _ = compute_fg(&x, &mut sys_b, &mut g_b);

    sys_b.eval.radius = unscaled.clone();
    sys_b.sync_atom_props();
    force_cache_miss(&mut sys_b);
    let f_b = compute_f(&x, &mut sys_b);
...
fn move_flag_true_bypasses_cache() {
    ...
    sys.eval.fdist_atom.iter_mut().for_each(|v| *v = 0.0);
    sys.eval.frest_atom.iter_mut().for_each(|v| *v = 0.0);
    let _ = compute_f(&x, &mut sys);
    let fdist_move_a = sys.eval.fdist_atom.clone();
    let frest_move_a = sys.eval.frest_atom.clone();
    ...
    sys2.eval.fdist_atom.iter_mut().for_each(|v| *v = 0.0);
    sys2.eval.frest_atom.iter_mut().for_each(|v| *v = 0.0);
    force_cache_miss(&mut sys2);
    let _ = compute_f(&x, &mut sys2);

    assert_eq!(
        fdist_move_a.len(),
        sys2.eval.fdist_atom.len(),
        "fdist_atom shape mismatch"
    );
    for (i, (&a, &b)) in fdist_move_a
        .iter()
        .zip(sys2.eval.fdist_atom.iter())
        .enumerate()
    {
        assert_eq!(
            a.to_bits(),
    ...
    for (i, (&a, &b)) in frest_move_a
        .iter()
        .zip(sys2.eval.frest_atom.iter())
        .enumerate()
    {
        assert_eq!(
            a.to_bits(),
